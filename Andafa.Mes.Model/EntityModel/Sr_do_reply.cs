using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Model.EntityModel
{
	/// <summary>
	/// sr_do_reply
	/// </summary>
	[Serializable]
	public class Sr_do_reply
	{
		/// <summary>
		/// sr_do_reply_id
		/// </summary>
		public string sr_do_reply_id
		{
			get;
			set;
		}
		/// <summary>
		/// supplier_no
		/// </summary>
		public string supplier_no
		{
			get;
			set;
		}
		/// <summary>
		/// supplier_name
		/// </summary>
		public string supplier_name
		{
			get;
			set;
		}
		/// <summary>
		/// part_no
		/// </summary>
		public string part_no
		{
			get;
			set;
		}
		/// <summary>
		/// part_name
		/// </summary>
		public string part_name
		{
			get;
			set;
		}
		/// <summary>
		/// part_spec
		/// </summary>
		public string part_spec
		{
			get;
			set;
		}
		/// <summary>
		/// part_idt
		/// </summary>
		public string part_idt
		{
			get;
			set;
		}
		/// <summary>
		/// part_unit
		/// </summary>
		public string part_unit
		{
			get;
			set;
		}
		/// <summary>
		/// mrp_region_no
		/// </summary>
		public string mrp_region_no
		{
			get;
			set;
		}
		/// <summary>
		/// do_date
		/// </summary>
		public DateTime do_date
		{
			get;
			set;
		}
		/// <summary>
		/// do_reply_qty
		/// </summary>
		public decimal do_reply_qty
		{
			get;
			set;
		}
		/// <summary>
		/// is_locked
		/// </summary>
		public bool? is_locked
		{
			get;
			set;
		}
		/// <summary>
		/// sr_do_reply_rmk01
		/// </summary>
		public string sr_do_reply_rmk01
		{
			get;
			set;
		}
		/// <summary>
		/// sr_do_reply_rmk02
		/// </summary>
		public string sr_do_reply_rmk02
		{
			get;
			set;
		}
		/// <summary>
		/// sr_do_reply_rmk03
		/// </summary>
		public string sr_do_reply_rmk03
		{
			get;
			set;
		}
		/// <summary>
		/// sr_do_reply_rmk04
		/// </summary>
		public string sr_do_reply_rmk04
		{
			get;
			set;
		}
		/// <summary>
		/// crt_time
		/// </summary>
		public DateTime crt_time
		{
			get;
			set;
		}
		/// <summary>
		/// crt_user
		/// </summary>
		public string crt_user
		{
			get;
			set;
		}
		/// <summary>
		/// crt_user_no
		/// </summary>
		public string crt_user_no
		{
			get;
			set;
		}
		/// <summary>
		/// crt_user_name
		/// </summary>
		public string crt_user_name
		{
			get;
			set;
		}
		/// <summary>
		/// crt_host
		/// </summary>
		public string crt_host
		{
			get;
			set;
		}
		/// <summary>
		/// upd_time
		/// </summary>
		public DateTime upd_time
		{
			get;
			set;
		}
		/// <summary>
		/// upd_user
		/// </summary>
		public string upd_user
		{
			get;
			set;
		}
		/// <summary>
		/// upd_user_no
		/// </summary>
		public string upd_user_no
		{
			get;
			set;
		}
		/// <summary>
		/// upd_user_name
		/// </summary>
		public string upd_user_name
		{
			get;
			set;
		}
		/// <summary>
		/// upd_host
		/// </summary>
		public string upd_host
		{
			get;
			set;
		}
	}
}